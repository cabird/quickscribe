...
class SpeakerProcessor:
    ...
    def _download_audio(self, recording: Recording,
        ...
        """Download recording audio to a temp file."""
        try:
            suffix = os.path.splitext(recording.unique_filename)[1] or ".mp3"
            tmp = tempfile.NamedTemporaryFile(suffix=suffix, delete=False)
            tmp.close()

            self.blob_client.download_file(recording.unique_filename, tmp.name)
            job_logger.info(f"Downloaded audio: {recording.unique_filename}", recording.id)
            return tmp.name
