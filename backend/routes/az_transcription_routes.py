...
def start_transcription(recording_id):
    ...
    if not recording or recording.user_id != user.id:
        logger.error(f"Recording not found or does not belong to the user: {recording_id}")
        return jsonify({'error': 'Recording not found or does not belong to the user'}), 404

    logger.info(f"found recording: {recording.original_filename}")
    ...
    if recording and recording.transcription_status in [TranscriptionStatus.in_progress, TranscriptionStatus.completed]:
        logger.warning(f"Transcription already exists or in progress for this recording: {recording.id}")
    ...
    if not transcription:
        ...
        logger.info(f"Transcription found: {transcription.id}")
    
    try:
        blob_sas_url = generate_recording_sas_url(recording.unique_filename)
        logger.info(f"SAS URL generated: {blob_sas_url}")        
        # Call the long-running Azure Speech Services transcription function asynchronously
        logger.info(f"Calling Azure Speech Services transcription function: {transcription.id}")
            
        logger.info(f"Submitting transcription to Azure Speech Services: {blob_sas_url}")
...
def check_in_progress_transcription(recording: Recording) -> tuple[str, str]:
    ...
    logger.info(f" got transcription for {az_transcription_id}")
    ...
    logger.info(f"AZ Transcription status: {status}")
    error_message = ""
    if status == TranscriptionStatus.completed.value:
        ...
        logger.info(f"Getting transcript: {az_transcription_id}")
        ...
        logger.info(f"Updating transcription: {transcription.id}")
        ...
        logger.info(f"Transcription failed: {az_transcription.properties.error}")
        ...
        logger.info(f"Transcription is still running: {az_transcription_id}")
...
def az_transcribe(blob_sas_url, original_filename):
    ...
    logger.info(f"Transcription created: {created_transcription}")
...
def az_get_transcript(az_transcription_id): 
    ...
    if transcription.status == "Succeeded":
        ...
        for file_data in az_paginate(api, pag_files):
            ...
            #logger.info(f"Results for {az_transcription_id}:\n{results.content.decode('utf-8')}")
